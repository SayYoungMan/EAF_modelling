...
F(1) = -x(1) + (Q_arc + Q_CH4 + K_post*dH_Th)*(1-K_sSclSc) + Q_lScsSc ...
    - Q_sScsSl - Q_sSclSl - Q_sScgas - Q_sScwater - x(10);

% Energy received by liquid metal zone (lSc)
F(2) = -x(2) + (Q_arc + Q_CH4 + K_post*dH_Th)*K_sSclSc + Q_lScchem ...
...
F(14) = -x(14) + (-x(8) + (A1/(A1+A2))*Q_gaswater - phi1*Cp_H2O*(T_roof - T_water)) ...
    / (A1*d1*rho*Cp_roof);

% Temperature change of wall
F(15) = -x(15) + (-x(9) + (A2/(A1+A2))*Q_gaswater - phi2*Cp_H2O*(T_wall - T_water)) ...
    / (A2*d2*rho*Cp_wall);

% Melt rate of solid scrap
F(16) = x(16) + (x(1)*(T_sSc/T_melt)) / (lambda_sSc + Cp_sSc*(T_melt - T_sSc));
