...
%         (Cp_CO2 + 2*Cp_H2O - Cp_CH4 - 2*Cp_O2) * (T_gas - 298));
    ...
    Q_sSc = (Q_arc + 25000)*(1-K_sSclSc) + Q_lScsSc ...
    ...
    V_sSc_melt = V_sSc_melt + (dm_sSc * ts)/rho_sSc;
    h_cone = (V_sSc_melt/(16.8*pi))^(1/3);
    d_conein = 3.79*h_cone;
    d_coneout = 4.4*h_cone;
...
% Graph generation
time = linspace(1, total, total);

gas_temp = movmean(gas_temp,50);
