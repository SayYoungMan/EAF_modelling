...
dT_sSl = (Q_sSl*(1-(T_sSl/T_melt))) / (m_sSl*Cp_sSl);
% Temperature change of lSl
dT_lSl = Q_lSl/(m_lSl*Cp_lSl);

% ------------------------ Mass changes ---------------------------

% Melt rate of solid slag
dm_sSl = -(Q_sSl*(T_sSl/T_melt)) / (lambda_sSl + Cp_sSl*(T_melt - T_sSl));
...
x2_d3 = -x1_d3;
x2_d4 = -kd_Mn1*(M_C/M_MnO)*(X_MnO - Xeq_MnO1);
...
F(1) = -x(1) + (Q_arc + Q_CH4 + K_post*x(28))*(1-K_sSclSc) + Q_lScsSc ...
    - Q_sScsSl - Q_sSclSl - Q_sScgas - Q_sScwater - x(10);

% Energy received by liquid metal zone (lSc)
F(2) = -x(2) + (Q_arc + Q_CH4 + K_post*x(28))*K_sSclSc + Q_lScchem ...
    - Q_lScsSc - Q_lScsSl - Q_lSclSl - Q_lScgas - Q_lScwater - x(11);

% Gas zone energy balance
F(3) = -x(3) + Q_arcgas + (1-K_post)*x(28) + Q_CH4gas + Q_sScgas + Q_lScgas ...
    + Q_lSlgas - Q_gaswater;

% Radiative heat transfer from roof
F(4) = x(4) - ep1*sig*T_roof^4 - (1-ep1)*(VF12*x(5) + VF13*x(6) ...
    + VF14*x(7) + VF15*Q_arcRAD);

% Radiative heat transfer from wall
F(5) = x(5) - ep2*sig*T_wall^4 - (1-ep2)*(VF21*x(4) + VF23*x(6) ...
    + VF24*x(7) + VF25*Q_arcRAD);

% Radiative heat transfer from Solid Metal
F(6) = x(6) - ep3*sig*T_sSc^4 - (1-ep3)*(VF31*x(4) + VF32*x(5) ...
    + VF35*Q_arcRAD);

% Radiative heat transfer from liquid metal
F(7) = x(7) - ep4*sig*T_lSc^4 - (1-ep4)*(VF41*x(4) + VF42*x(5) ...
    + VF45*Q_arcRAD);

% Radiative heat flow in roof
F(8) = -x(8) + A1 * (VF12*(x(4)-x(5)) + VF13*(x(4)-x(6)) ...
    + VF14*(x(4)-x(7))) - VF51 * Q_arcRAD;

% Radiative heat flow in wall
F(9) = -x(9) + A2 * (VF21*(x(5)-x(4)) + VF23*(x(5)-x(6)) ...
    + VF24*(x(5)-x(7))) - VF52 * Q_arcRAD;

% Radiative heat flow in sSc
F(10) = -x(10) + A3 * (VF31*(x(6)-x(4)) + VF32*(x(6)-x(5))) ...
    - VF53 * Q_arcRAD;

% Radiative heat flow in lSc
F(11) = -x(11) + A4 * (VF41*(x(7)-x(4)) + VF42*(x(7)-x(5))) ...
    - VF54 * Q_arcRAD;

% Temperature change of sSc
F(12) = x(12) - (x(1)*(1-(T_sSc/T_melt))) / (m_sSc*Cp_sSc);

% Temperature change of lSc
F(13) = x(13) - x(2)/(m_lSc*Cp_lSc);

% Temperature change of gas
F(14) = x(14) - x(3)/(m_gas*Cp_gas);

% Temperature change of roof
F(15) = -x(15) + (-x(8) + (A1/(A1+A2))*Q_gaswater - phi1*Cp_H2O*(T_roof - T_water)) ...
    / (A1*d1*rho*Cp_roof);

% Temperature change of wall
F(16) = -x(16) + (-x(9) + (A2/(A1+A2))*Q_gaswater - phi2*Cp_H2O*(T_wall - T_water)) ...
    / (A2*d2*rho*Cp_wall);

% Melt rate of solid scrap
F(17) = x(17) + (x(1)*(T_sSc/T_melt)) / (lambda_sSc + Cp_sSc*(T_melt - T_sSc));

% Liquid metal mass rate
F(18) = x(18) + x(17);

% Rate of change of CO
F(19) = x(19) + ((k_PR*x(27)*m_CO)/(m_CO + m_CO2 + m_N2 + m_O2));
F(20) = x(20) - x9_d5 - x(19) - x9_d2 - x9_d1;

% Rate of change of CO2
F(21) = x(21) + (k_PR*x(27)*m_CO2) / (m_CO+m_CO2+m_N2+m_O2);
F(22) = x(22) - x10_d8 - x(21) - x10_d6 - x10_d5 - x10_d4 - x10_d3 - x10_d1;

% Rate of change of N2
F(23) = x(23) + (k_PR*x(27)*m_N2) / (m_CO+m_CO2+m_N2+m_O2);
F(24) = x(24) - x(23) - x11_d1;

% Rate of change of O2
F(25) = x(25) + (k_PR*x(27)*m_O2) / (m_CO+m_CO2+m_N2+m_O2);
F(26) = x(26) - x12_d8 - x(25) - x12_d6 - x12_d5 - x12_d4 - x12_d1;

% Relative Pressure
F(27) = -x(27) + (R*T_gas/V_gas) * (x(20)/M_CO + x(22)/M_CO2 + ...
    x(24)/M_N2 + x(26)/M_O2) + (R*x(14)/V_gas) * (m_CO/M_CO + m_CO2/M_CO2 ...
    + m_N2/M_N2 + m_O2/M_O2);

% h) CO + 1/2O2 -> CO2
F(28) = x(28) - (x(19)/M_CO) * ((dH_CO2-dH_CO) + (Cp_CO2 - Cp_CO - 0.5*Cp_O2)*(T_gas-298));
end
